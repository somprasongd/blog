...
ในบทความนี้จะใช้ระบบฐานข้อมูลเป็น [PostgreSQL](https://www.postgresql.org/) เริ่มจากการติดตั้ง database driver [pq](https://github.com/lib/pq) และ library [sqlx](http://jmoiron.github.io/sqlx/)
...
การใช้งาน [sqlx](http://jmoiron.github.io/sqlx/) เบื้องต้นสามารถใช้งานได้เหมือน `database/sql` เลย แค่เปลี่ยนจาก `sql` เป็น `sqlx` ก็สามารถใช้งานได้เลย โดยไม่ต้องแก้ไขโค้ดส่วนอื่นๆ
...
ตัว [sqlx](http://jmoiron.github.io/sqlx/) ได้เตรียม `sqlx.Connect()` ไว้ให้โดยทำงานทั้งเชื่อมต่อ และตรวจสอบโดยใช้ ping ให้เรียบร้อยแล้ว
...
การค้นหาข้อมูลสามารถใช้ `db.Query()` ได้เหมือน `database/sql` หรือจะเปลี่ยนเป็น `db.Queryx()` ของ [sqlx](http://jmoiron.github.io/sqlx/) ก็ได้ แต่ใช้ `db.Select()` ดีกว่า เพราะจะช่วยเอาข้อมูลใส่ Slice ให้เลย
...
ตัว database driver https://github.com/lib/pq นั้น ไม่สามารถใช้ named parameters ได้ แต่ตัว [sqlx](http://jmoiron.github.io/sqlx/) มีความสามารถนี้เพิ่มมาให้ โดยใช้ฟังก์ชัน `db.NamedQuery()`
...
การค้นหาจาก Id ถ้ามีข้อมูลจากได้ออกมาแค่ 1 row จะใช้ `db.QueryRow()` ได้เหมือนเดิม หรือจะเปลียนมาเป็น `db.QueryRowx()` ของ [sqlx](http://jmoiron.github.io/sqlx/) ก็ได้ แต่ใช้ `db.Get()` ดีกว่าเพราะจะได้ค่าใส่ struct กลับมาให้เลย
...
แต่ตัว [sqlx](http://jmoiron.github.io/sqlx/) มีฟังก์ชัน `db.NamedExec()` มาให้ ซึ่งจะเป็นแบบ named parameters
...
ก็จบแล้วสำหรับการทำ Select, Insert, Update และ Delete ลงฐานข้อมูลโดยใช้ [sqlx](http://jmoiron.github.io/sqlx/) แทน standard library สามารถศึกษาเพิ่มเติมได้จาก [http://jmoiron.github.io/sqlx/](http://jmoiron.github.io/sqlx/)
