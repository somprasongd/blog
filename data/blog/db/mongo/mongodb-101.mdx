...
draft: false
summary: 'MongoDB เป็นระบบฐานข้อมูลแบบ NoSql ซึ่งจะจัดเก็บข้อมูลในรูปแบบ document ที่เรียกว่า BSON (คล้าย JSON objects)'
---
...
<div className="flex justify-center">
  <div>![MongoDB Document](/static/images/db/mongo/101/crud-annotated-document.png)</div>
</div>

โดย data records จัดจะเก็บเป็น BSON Documents ซึ่งจะอยู่ใน Collections (เทียบเท่า Table) ที่อยู่ใน databases แต่ละตัว

<div className="flex justify-center">
  <div>![Collections](/static/images/db/mongo/101/crud-annotated-collection.png)</div>
</div>
...
- การจะใช้แสดงข้อมูลใน element ไหนใน Array ใช้ `$slice` projection operator `db.users.find( { status: "A" }, { name: 1, status: 1, points: { $slice: -1 } } )` คือให้เอา element สุดท้ายออกมา **_จำไว้ว่าไม่สามารถระบุเป็น index แบบนี้ ได้ `{ "ratings.0": 1 }`_**
...
4. `db.collection.update(<filter>, <update>, {upsert: <boolean>, multi: <boolean>, writeConcern: <document>})` Either updates or replaces a single document that match a specified filter or updates all documents that match a specified filter. _โดยปกติ update() จะเป็นการอัพเดทรายการเดียว ถ้าต้องการหลายรายการใช้ multi option `{ multi: true }`_
...
<div className="flex justify-center">
  <div>![Aggregation Pipeline](/static/images/db/mongo/101/aggregation-pipeline.png)</div>
</div>
